...
chunk-worker.js/vendor)도 같은 서버·같은 포트에서 함께 서빙합니다** — 별도로 프런트 정적
...
- 프런트엔드 정적 파일 서빙 (`GET /`, `/app.js`, `/chunk-worker.js`, `/vendor/*` 등)
- 서버 프로필 목록/생성/삭제
- Bridge 연결 테스트 엔드포인트
- Bridge 레지스트리 프록시 엔드포인트
- Bridge 청크 프록시 엔드포인트
- 서버 콘솔 SSE 프록시(`GET /bridge/console/stream`) + 명령어 실행(`POST /bridge/console/command`) —
  브라우저 EventSource가 커스텀 헤더를 못 보내는 문제 때문에 프런트는 이 프록시로만 붙고,
